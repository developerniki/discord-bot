...
class TicketStore(BaseStore):
    ...
    async def create_ticket(self, guild_id: int, user_id: int, reason: Optional[str] = None) -> Ticket:
        ...
        ticket = Ticket(ticket_id=lastrowid, guild_id=guild_id, user_id=user_id, reason=reason, status="open",
    ...
    async def get_open_tickets_by_user(self, guild_id: int, user_id: int) -> int:
        query = """SELECT COUNT(*)
                   FROM Tickets
